...
///-----------------------///
router.post(
  "/posts/:id/codepreciate",
  authMiddleware.isLoggedIn,
  authMiddleware.isNotPostOwned,
  (req, res) => {
        ...
        if (!apreciated) {
          ...
          post.save((err, post) => {
            if (err || !post) {
              ...
              req.flash(
                flashMessages.codepreciated.type,
                flashMessages.codepreciated.message
              );
          ...
          post.save((err, post) => {
            if (err || !post) {
              ...
              req.flash(
                flashMessages.notCodepreciated.type,
                flashMessages.notCodepreciated.message
              );
              res.redirect("/posts/" + req.params.id);
            }
          });
        ...
        if (err || !comment) {
          ...
          post.save((err, post) => {
            if (err || !post) {
              ...
              req.flash(
                flashMessages.addedComment.type,
                flashMessages.addedComment.message
              );
              res.redirect("/posts/" + req.params.id);
            }
          });
...
async function DeleteComment(req, res, comment, post) {
  Comment.findByIdAndDelete(comment, (err) => {
    if (err) {
      ...
      req.flash(
        flashMessages.commentDeleted.type,
        flashMessages.commentDeleted.message
      );
...
router.delete(
  ...
  authMiddleware.isLoggedIn,
  authMiddleware.isCommentOwnedOrOwner,
  (req, res) => {
