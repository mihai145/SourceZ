...
///-----------------------///
router.get(
  "/problemset/submissions/:id",
  authMiddleware.isLoggedIn,
  authMiddleware.submissionAuth,
  (req, res) => {
...
router.post("/problemset/newPb", authMiddleware.isOwner, (req, res) => {
  ...
  Problem.create(prob, (err, problem) => {
    if (err || !problem) {
      ...
      req.flash(
        flashMessages.defaultSuccess.type,
        flashMessages.defaultSuccess.message
      );
      res.redirect("/problemset");
    }
  });
...
router.get("/problemset/:problemName", (req, res) => {
  Problem.findOne({ name: req.params.problemName }, (err, problem) => {
    if (err || !problem) {
      ...
      if (
        Date.now() < problem.visibleFrom &&
        !(req.isAuthenticated() && req.user.isOwner)
      ) {
        req.flash(
          flashMessages.defaultFail.type,
          flashMessages.defaultFail.message
      ...
      if (!req.isAuthenticated()) {
        ...
        Submission.find({
          ...
          toProblem: req.params.problemName,
        })
          .sort({ created: -1 })
          .limit(4)
          .then((submissions) => {
...
function SubmitProblem(req, res, toContest, problemNumber) {
  ...
  if (diffMins < 2 && req.user.lastSubmission !== null) {
    req.flash(
      "fail",
      "You submitted a solution less than two minutes ago. You can only submit once every two minutes"
    );
    ...
    if (req.body.clientSource.indexOf("remove") !== -1) {
      req.flash(
        "fail",
        "Remove is a reserved keyword and it cannot be used in source code"
      );
      return res.redirect("/problemset/" + req.params.problemName);
    } else if (req.body.clientSource.indexOf("system") !== -1) {
      req.flash(
        "fail",
        "System is a reserved keyword and it cannot be used in source code"
      );
      return res.redirect("/problemset/" + req.params.problemName);
    } else if (
      req.body.clientSource.indexOf('"' + req.params.problemName + '.in"') == -1
    ) {
      req.flash(
        "fail",
        "Input file named " +
          req.params.problemName +
          ".in " +
          " not detected. Try again..."
      );
      return res.redirect("/problemset/" + req.params.problemName);
    } else if (
      req.body.clientSource.indexOf('"' + req.params.problemName + '.out"') ==
      -1
    ) {
      req.flash(
        "fail",
        ...
        if (err || !user) {
          ...
          Submission.create(submission, (err, subm) => {
            if (err || !subm) {
              ...
              req.flash(
                flashMessages.successfullySubmited.type,
                flashMessages.successfullySubmited.message
              );
              res.redirect("/problemset/" + req.params.problemName);
            }
          });
          ...
          Contest.findOne({ title: problem.fromContest }, (err, contest) => {
            if (Date.now() < contest.finishDate) {
              ...
              Registration.findOne(
                { contestant: req.user.username, contest: problem.fromContest },
                (err, reg) => {
                  if (err || !reg) {
                    req.flash(
                      "fail",
                      "You are not registered to this contest. You can try this problem after the contest ends."
                    );
                    return res.redirect("/problemset");
                  } else {
...
function FetchSubmissions(req, res) {
  if (req.user.isAdmin) {
    Submission.find({ toProblem: req.params.problemName })
      .sort({ created: -1 })
      .exec((err, submissions) => {
        if (err || !submissions) {
          ...
          res.render("problemset/allSubm", {
            problem: req.params.problemName,
            submissions: submissions,
          });
        }
      });
...
router.get(
  "/problemset/:problemName/allSubm",
  authMiddleware.isLoggedIn,
  (req, res) => {
    if (!req.user.isOwner) {
      Problem.findOne({ name: req.params.problemName }, (err, problem) => {
